...
const EventCard = (course, currentSchedule) => {
  ...
  var smallClass;
  if (
    Number(selectedCourse.endTime.slice(0, 2)) -
      Number(selectedCourse.startTime.slice(0, 2)) <
    2
  ) {
    smallClass = true;
  }
  ...
  return (
    <Box
      ...
      style={{ flexDirection: smallClass ? 'row' : 'column' }}
    >
      <div
        style={{
          display: 'flex',
      ...
      >
        ...
        <Typography
          variant="body"
          style={{ display: smallClass ? 'none' : 'flex' }}
          className={classes.text}
        >
